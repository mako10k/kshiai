# Character semantic migration — requirement candidate revision 5

- Status: Step 1 candidate; self-review complete; first owner review pending
- Date: 2026-09-10
- Decision owner: Product owner
- Replaces for review: `character-v2-compatibility-requirements-v4.md`
- Sources: owner correction after revision-4 independent review; revision-4
...
Revision 5 intentionally changes ADR-0010's deterministic-only legacy mapping
direction. A successor ADR must explicitly authorize a bounded LLM-assisted
semantic migration while retaining frozen input identity, provider-attempt
...
ADR-0011 and stay readable. Revision 5 does not reinterpret their stored bytes.
...
for semantic migration. Requirement revisions 1 through 4 remain historical and
...
compatibility. No LLM call occurs on a selection, search, battle-create, retry,
resume, or replay read path.
...
R9. Each migration attempt freezes the complete V2 source, natural-source material
...
identity, and request digest. Provider chain-of-thought is neither requested nor
stored. Provider failure or invalid output leaves the current generation
unchanged and the attempt retryable.
...
marker, bounded owner-facing explanation, provenance category, and affected
...
R14. Structural validation covers the complete candidate and capsule: schema
bounds, registered references, action legality, compiler compatibility,
disclosure ceilings, consumer access, target/source completeness, operation
coverage, and no unaccounted source removal. An invalid structured response is
retried with the error, cause, relevant source context, and prior valid result.
The retry requests the invalid and semantically affected portions only, then
revalidates the complete merged candidate. It does not require wholesale output
when a bounded repair is sufficient.
...
candidate digests, and capsule. Later separately authorized revisions may move
the current pointer; the migration receipt remains historical and does not claim
that its target is still current.

R22. Idempotency binds source generation, migration contract, request digest, and
accepted candidate identity. Repeating a completed attempt returns its recorded
target without a duplicate generation or provider call. Retrying invalid fragments
continues the same persisted attempt. Intentionally asking the provider for a
different semantic result creates a new attempt and owner-review candidate rather
than silently replacing the prior output. Pointer drift stops activation without
overwriting newer owner work.
...
- Invalid provider output exercises bounded patch retry for the invalid and
  semantically affected portions; the merged complete candidate is revalidated.
- Same-attempt retry reuses persisted valid output and creates neither a second
  provider operation nor generation; requested regeneration creates a distinct
  attempt and review candidate.
...
dead ends. Revision 5 does not select it as the complete migration strategy.
...
activation controls. Revision 5 rejects it.
...
permanent reader exception and deterministic-map dead ends. Revision 5 selects
this option.
...
- Migration prompt, response, validation, and bounded patch contract identities
...
Review the exact revision-5 candidate for: correct authority refinement of
...
repair; idempotency despite nondeterministic generation; owner acceptance;
