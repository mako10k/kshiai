# Generative and recoverable V3 character authoring — requirement candidate revision 5

- Status: Step 1 candidate; self-review complete; owner review pending
- Date: 2026-09-11
- Decision owner: Product owner
- Replaces for review: `character-v3-authoring-requirements-v4.md`
- Scope: V3 character creation, V3 character revision, and V2-to-V3 migration
- Sources: pending character revision 4; accepted structured semantic authoring foundation
  revision 3 and its owner acceptance record; owner clarification on source-based retry
...
This character requirement adopts F1-F15 of the accepted
[structured semantic authoring foundation revision 3](structured-semantic-authoring-foundation-requirements-v3.md),
exact SHA-256 b5693f6d47e975ea1cb78adaa686185630db5ffc6d52c74f3481ac77511c99da.
Its [acceptance record](structured-semantic-authoring-foundation-requirements-v3-acceptance.md)
...
Revision 4 remains a historical pending candidate. This revision replaces it for
review, inheriting foundation v3 mechanical safety and defining character progress
signals. It preserves v4 source-based retry and migrator-only preservation rules.
...
The review scope is this complete revision 5 and its difference from revision 4.
...
- The accepted foundation revision 3 specific prior-contract disposition governs
...
Semantic review uses four proportionate stages. The blocking rules below concern
semantic findings, not permission to bypass foundation F15 hard stops or R16 complete
validation. Untrustworthy internal state is not an ordinary rejected patch:
...
Within this semantic classification, only the last four block candidate-ready and
create focused repair or, when R13 is satisfied, human questions. Foundation F15
hard stops and R16 validation remain independently mandatory. Schema validity alone
cannot override reconciliation.
...
Inherit foundation F9/F11/F15. Missing information and minor contradictions normally
...
execution budgets may end an attempt as failed. Foundation F15 additionally requires
failure when its hard-stop or unresolved bounded-stall/internal-corruption conditions
hold; further execution and late-result application must cease as specified there.
...
### R21. Character progress and inherited mechanical safety

Use foundation F15 for all character create/revise/migrate attempts; do not implement
a second character-specific budget or give character resolvers authority to reset
counters, expand ceilings, or override a terminal failure.

The versioned character adapter supplies phase-aware progress and relevant-state
equivalence based on existing domain contracts:

- skeleton work: accounted protected identity, goal, essential ability/limitation,
  and relationship claims, with their unresolved material conflicts;
- dependent work: resolved required V3 obligations, source dispositions, and legal
  references, including dependencies reopened by material changes;
- reconciliation: covered semantic lenses and resolved material discrepancies against
  frozen input and authorized changes.

Signals refer to validated changes and accounting evidence, not raw prose growth,
call counts, or the LLM declaring that the character is complete. They do not define
a new scalar quality score or prove semantic correctness. Defining exactly how to
compare them within each phase remains successor design under foundation F2/F15.

Repairs may temporarily reopen obligations or increase findings; that is not itself
a stall. Repeatedly alternating a goal, ability, or relationship interpretation
without resolving relevant obligations must be observable as equivalent/recurrent
states despite irrelevant prose changes. Detect and recover within the inherited
bounded windows and budgets; no indefinite exploration or every-step improvement
requirement is introduced.

Character conformance must exercise cumulative budgets across planning, lenses,
decomposition and repair; repeated and alternating unresolved states; useful
temporary regression; rejected proposals versus corrupt trusted ledgers; and
terminal-failure handling of late results and explicit source-based retry.
Controlled adapters may verify these safety behaviors without paid provider calls;
actual automatic completion and semantic quality remain R20 evaluation obligations.

In-memory bounded monitoring history is sufficient. Retain only the compact
failure diagnostics required by foundation F15 and existing receipts; full
conversation or intermediate migration context persistence is not required.
This authoring progress contract does not move knowledge or deliberation into the
reaction-only psyche, or change conscious judgment or engine responsibilities.
...
20. Character attempts inherit foundation F15 without duplicate or resettable budgets.
21. Adapter progress/state-equivalence signals cover character skeleton, dependent
    obligations and reconciliation, while allowing legitimate temporary regressions.
22. Character conformance covers bounded stagnation/cycles, corruption versus invalid
    proposals, late results, and explicit source-based retry without mandatory checkpoints.
...
## Revision 4 disposition and self-review

Revision 4 is preserved unchanged. This candidate updates the fixed foundation
dependency from accepted v2 to accepted v3 and inherits F15.

- R1-R9, R11-R14, R16-R18, R20 and the three mode completion contracts retain their
  character outcomes and wording.
- R10 and R15 clarify that semantic blocking classifications never override mechanical
  hard stops or complete validation. They introduce no new semantic rejection class.
- R19 incorporates F15 terminal-failure conditions.
- R21 supplies domain progress meaning and safety conformance; AC20-22 cover it.
- Existing accepted repair ceilings, source-based retry, preservation access,
  protected meaning and runtime responsibility boundaries remain unchanged.

Self-review: requirement revision 5 is not a product/schema version change.
No new numerical ceiling, API, module, detector algorithm or provider route is chosen.
Independent review and owner acceptance are pending; existing implementation is not
evidence of conformance.

The alternative is only updating the foundation reference. That is smaller but leaves
the character adapter progress contract implicit and the semantic-only blocking
wording ambiguous. The selected explicit alignment adds a small domain contract,
not a separate controller. Detector false positives and missed cycles remain
evaluation risks; foundation limits remain independent of detector accuracy.

## Proposed independent-review input

Review this complete revision5 against exact accepted foundation revision3 and pending
character revision4. Verify R21 domain progress signals preserve character semantics,
remain separate from runtime psyche/conscious/engine responsibilities, and do not
require monotonic every-step progress or intermediate-context persistence. Check that
R10/R15 semantic blocking and R19 failure handling respect F15, without converting
sparse generation or minor reconciliation into early rejection. Verify all three
mode outcomes, protected meaning, migration-only capsule access, existing ceilings,
source-based retry, final strict validation and exceptional R13 Q&A remain intact.
Classify unresolved numeric/detector/provider choices as later design/evaluation,
not invented acceptance criteria. REVIEW leads to independent review then Step4;
candidate edits require a new revision and renewed Step1.
