...
9. サーバー: 語り視点または既存fluid focusに対応する一時的ナレーションviewと、その視点に許可された表示専用profile anchorを生成
10. ナレータLLM: 許可されたview・表示専用profile anchor・内面digest・採用済み実発話から、人物像に矛盾しない地の文と実発話の配置・許容された表層加工だけを生成し、台詞・属性・状態を新規決定しない
11. サーバー: ナレータによる台詞の追加・欠落・事実変更を拒否して原文へ戻し、既知IDを表示ラベルへ置換する。実発話をキャラの継続状態とturn record、公開レンダリングをログに分離して、最新状態・frame・current registry・短いDramaStateを保存する
```
