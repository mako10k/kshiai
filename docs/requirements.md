...
| F-BTL-13 | 各キャラは戦闘ごとに独立した非公開エージェント状態（短い継続メモ・目標・感情・認識・話し方・一人称）を持ち、ターン間で保存する。新しいCompact契約では完了済み発話を意味状態へ複製せず、正準utterance eventから作るobserver別の有界な`utteranceHistory`として分離する。`lastSpeech`はV1契約の既存Battleを再生するためだけに読み取り可能とし、V2契約では入力にも新規書込みにも使わない | Must |
...
| F-BTL-15 | 各キャラエージェントは自分の非公開継続状態と最新知覚frameから、更新済み非公開状態・次行動・実際に発した台詞または反応を生成する。V2 Compact表現境界は意味状態を`expressionState`、完了実績を`utteranceHistory`、今回の出力を`nextUtterance`として分離し、同一本文を含むすべての有効な出力を本文比較なしで同じ採用経路へ渡す。ナレータは採用済み実発話と確定イベントを受け取り、台詞の配置と事実・意図・話者・発話種別を変えない表層加工、および地の文だけを担当する。検証できない加工は実発話原文へ戻し、公開表示をキャラ状態へ書き戻さない。戦闘効果と勝敗は引き続きエンジンのみが確定する | Must |
