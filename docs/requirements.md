...
| F-BTL-11 | 戦闘開始時のシート値を基準値として保持し、2ターン目以降は全パラメータを毎ターン差分の20%（最低1）ずつ基準値へ戻す | Must |
| F-BTL-12 | 復元対象にはHP・MP・スタミナと各最大値を含める。現ターン中にHPが尽きた場合は、次ターンの復元前に決着させる | Must |
...
5. LLM: ナレータ文 + キャラセリフ生成（結果を入力として）
6. エンジン: 終了条件判定 → 続行 or 決着
```
...
| 台詞作家 | 同上 + キャラ性格 | `「…」` 形式のセリフ + 話者明示 | 誰の台詞か不明な出力 |
...
| ID | 要件 | 優先度 |
|----|------|--------|
| F-CFG-01 | LLM プロバイダを xAI / Venice から選択可能（環境変数 + サーバー設定） | Must |
