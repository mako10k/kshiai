...
7. キャラLLM（各キャラ独立・並列）: 自分の知覚frameと非公開継続状態から内面・次行動を更新
8. サーバー: 語り視点または既存fluid focusに対応する一時的ナレーションviewを生成
9. ナレータLLM: 許可されたviewと内面digestから公開台詞と地の文を生成するだけで、状態を変更しない
10. サーバー: 既知IDを表示ラベルへ置換し、最新状態・frame・current registry・短いDramaStateを保存して過去frameを蓄積しない
```
...
| ナレータ | 確定ターン結果 + 語り視点別ナレーションview + 許可された内面digest + IDごとの表示ラベル | 地の文 + 公開台詞 | 状態変更、生パラメータ表の読み上げ、許可されない内面の参照、制御IDの文章出力 |
| キャラエージェント（各キャラ独立） | 自分のプロフィール + 非公開継続状態 + self/counterpart明示の最新知覚frame + 使用可能行動 | 更新済み非公開状態 + 非公開反応サンプル | 戦闘効果の確定、未識別対象の正体推定を事実化、公開台詞の直接確定、逐語的思考過程の保存・公開、他キャラ専用観測の直接参照 |
