...
| F-CHR-09 | キャラ画像を AI 生成できる（プロバイダ経由）。未生成時はプレースホルダ | Should |
...
| F-BTL-14 | エンジンは確定済み状態の構造化差分と各キャラ視点の認知スナップショットを生成・保存し、それだけを対応するキャラエージェントへ注入する。逐語的な思考過程は保存・公開しない | Must |
| F-BTL-15 | キャラの台詞は各キャラエージェントが生成し、ナレータは台詞を生成・改変しない。戦闘効果と勝敗は引き続きエンジンのみが確定する | Must |

#### 4.4.1 ターン・パイプライン（論理）

```text
1. エンジン: 行動可能チェック、行動順（spd 等）
2. プレイヤー or AI: 行動選択（技 / 防御 / 待機 等）
3. LLM (optional): シーン・状況の更新提案 + 係数マップ
4. エンジン: 係数の clamp / 許可キー検証 → 効果計算 → 状態更新
5. エンジン: 前後状態の構造化差分と各キャラ視点の認知情報を記録
6. キャラLLM（各キャラ独立）: 非公開継続状態を更新 + 自分のセリフを生成
7. ナレータLLM: 確定イベントから地の文のみ生成（キャラセリフは改変しない）
8. エンジン: 終了条件判定 → 続行 or 決着
```
...
| ナレータ | 確定したターン結果（抽象化イベント） | 地の文ナレーション | 生パラメータ表の読み上げ |
| キャラエージェント（各キャラ独立） | 自分のプロフィール + 非公開継続状態 + エンジン生成の認知情報 | 更新済み非公開状態 + 自分のセリフ | 戦闘効果の確定、逐語的思考過程の保存・公開、他キャラ状態の直接参照 |
...
| 画像 | xAI 画像 or Venice 画像（実装時にアダプタ） | キャラ画像生成 |
...
- アプリケーションコードは `LlmProvider` インタフェース経由のみ呼び出す
