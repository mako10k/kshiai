...
| F-BTL-05 | ターン上限（設定可能、既定例: 20）に達したら LLM 総合評価で勝者を決定 | Must |
| F-BTL-06 | 双方同時に戦闘不能などの例外はルール表で定義（例: 同時撃破は引き分け or 後攻判定） | Must |
...
| F-BTL-14 | エンジンは各キャラ別の最新観測スナップショットと直前revisionからの観測差分だけを生成・保存し、それを対応するキャラエージェントへ注入する。古い主観的継続性は必要な場合だけキャラの有界な内面記憶へ要約し、逐語的な思考過程は保存・公開しない | Must |
| F-BTL-15 | キャラエージェントは非公開の継続状態と反応サンプルを生成し、ナレータは語り視点で許可された内面digestと確定イベントから公開台詞・地の文を生成する。戦闘効果と勝敗は引き続きエンジンのみが確定する | Must |
...
| F-BTL-25 | BattleStateは最新セマンティックスナップショット、直前revisionからの最新遷移、A/B/publicの最新観測と観測差分だけを保持する。ターン記録へ状態・パッチ・内面差分を複製せず、旧バトルはLLM移行なしの決定論的revision-0状態で読み込める | Must |
| F-BTL-26 | 観測可否は構造化されたvisibleTo、所在、確定差分からエンジンが決定論的に投影し、自然文から「見えた」「隠れた」等をパターン判定しない。各観測は必須キャラ、最新変更、所持・付着、直近更新を優先して有界化する | Must |
| F-BTL-27 | ナレータは確定済み行動から構築した短い行動beat、技・通常攻撃の説明、直近の公開台詞・地の文を受け取り、開始・接触・結果が分かる行動中心の地の文を生成する。同文・同義の台詞反復を避ける | Must |
...
| F-BTL-30 | 10ターン目以降はspecial技を必殺技候補として解禁・優先し、決着圧と決定論的クリティカル機会を段階的に高める。補正は10ターン目で通常倍率、20ターン目で設定上限に達し、20ターン未満の上限設定でも同じ傾斜を保つ | Must |

#### 4.4.1 ターン・パイプライン（論理）

```text
1. エンジン: 行動可能チェック、行動選択、安定した行動IDの記録
2. エンジン: 許可済み係数を検証し、効果計算・終了条件・機械状態を確定
3. セマンティック調停LLM: 確定行動・イベントと変更前世界からJSON Pointer差分を提案
4. サーバー: revision・パス・参照・サイズ・保護領域を検証し、差分を原子的に適用
5. サーバー: 適用済み世界からA/B各視点の認知スナップショットを生成・記録
6. キャラLLM（各キャラ独立・並列）: 自分の非公開継続状態を更新
7. ナレータLLM: 確定イベント・許可済み内面digest・最終観測可能状態から公開台詞と地の文を生成するだけで、状態を変更しない
8. サーバー: 最新スナップショット、最新差分、確定行動・イベント、A/Bの最新認知、短いDramaStateを保存し、過去ターンへ状態コピーを蓄積しない
```
...
| ナレータ | 確定ターン結果 + 最終観測可能状態 + 語り視点で許可された内面digest | 地の文 + 公開台詞 | 状態変更、生パラメータ表の読み上げ、許可されない内面の参照 |
| キャラエージェント（各キャラ独立） | 自分のプロフィール + 非公開継続状態 + 機械的認知 + 自分向け最新観測スナップショット/差分 | 更新済み非公開状態 + 非公開反応サンプル | 戦闘効果の確定、公開台詞の直接確定、逐語的思考過程の保存・公開、他キャラ専用観測の直接参照 |
| 審判（上限時） | 全ターン要約 | 勝者・理由（物語調） | エンジン終了条件の上書き（上限時のみ権限） |
