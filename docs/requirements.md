...
| F-BTL-54 | 正準speechはキャラAgent由来の本文とサーバー所有のsource Sideを保持する。ナレータは現在viewの帰属確度と固定済み戦闘内ラベルからサーバーが列挙した表示話者ラベルだけを選べる。候補外、source Side不一致、事実を変える加工は既定ラベルと実発話原文へ戻し、表示ラベルをキャラ認知へ書き戻さない | Must |
...
9. サーバー: A/B双方の知覚frameと非公開状態の構造化結論から、A視点用・B視点用ナレータ継続状態を現在選択視点にかかわらず両方更新し、読者用公開継続状態とは分離する
10. サーバー: 語り視点または既存fluid focusに対応する一時的ナレーションview、該当するナレータ継続状態、その視点に許可された表示専用profile anchor、正準話者Sideごとの表示ラベル候補を生成
11. ナレータLLM: 許可されたview・表示専用profile anchor・内面digest・表示継続状態・採用済み実発話から、人物像に矛盾しない地の文と実発話の配置・候補内の話者ラベル・許容された表層加工だけを生成し、台詞・属性・状態を新規決定しない
12. サーバー: ナレータによる台詞の追加・欠落・事実変更・候補外話者ラベルを拒否して既定ラベルと原文へ戻し、既知IDを表示ラベルへ置換する。実発話をキャラの継続状態とturn record、公開レンダリングをログに分離して、最新状態・frame・current registry・A/Bナレータ継続状態・短いDramaStateを保存する
```
...
| ナレータ | 確定ターン結果 + 語り視点別ナレーションview + 視点で絞った表示専用profile anchor + 該当する表示専用ナレータ継続状態 + 許可された内面digest + 採用済み実発話 + 正準Side別の有限な表示話者ラベル候補 + IDごとの表示ラベル。上限判定時は確定済み裁定 + 直近の公開ナレーション | profileと矛盾しない地の文 + 実発話の配置・候補内の話者ラベル・事実不変な表層レンダリング。上限判定時は裁定を不変のまま囲むユーザー向け表現 | anchorだけを根拠にした属性公開、未設定属性の推測、台詞の新規生成・欠落、正準話者Side・事実・意図・肯否・対象・発話種別の変更、候補外話者ラベル、裁定勝者・理由の変更、キャラ状態への書戻し、状態変更、生パラメータ表の読み上げ、許可されない内面の参照、制御IDの文章出力 |
