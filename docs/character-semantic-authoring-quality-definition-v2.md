# キャラクター意味オーサリング 品質属性・レビュー十分性定義 revision 2

- 状態: Step 1候補、セルフレビュー済み、初回オーナーレビュー待ち
- 日付: 2026-09-15
- 決定者: プロダクトオーナー
- 置換候補: revision 1（受入済みではない）
...
各reviewの開始前に、3章の8項目すべてをreview inputへ記録する。表にない境界、測定量、
証拠方法、結果は、受入済みsourceまたは明示的なowner decisionから埋める。決まらない項目は
`判定不能`とし、reviewerが創作しない。結果はreview後にだけ記録する。
...
11. profile行はclaim templateであり、review inputで8項目を完成させてから判定する。
...
これらは実評価開始前の別owner決定で固定する。値の選択は本候補revision 2の受入範囲外である。

## 9. revision 1からの変更

独立レビューF1〜F3だけを反映した。

- 以前のreview statusと検査証拠を分け、新snapshotの全claimを現在形で判定する。
- 既存証拠はcurrent-fit確認後に再利用し、新規検査実行だけを影響範囲へ限定する。
- `免除`結果を削除し、適用外は固定scope、必須claimの変更は元のauthorityへ戻す。
- profile行をclaim templateとし、review inputで8項目を完成させる。

品質属性の定義、現profileの基準template、未決数値、対象範囲は変更していない。

## 10. セルフレビュー

- source provenance: 受入済み3要件、ADR-0031、revision 1と独立レビューを参照した。
- prior authority: 既存要件を補足し、上書きしない。revision 1は未受入の履歴として維持する。
- finding closure: F1は全claim current disposition、F2は免除削除、F3はtemplate具体化で処理した。
...
固定snapshotについて、revision 1独立レビューF1〜F3が閉じているかを確認する。特に、全claimの
current dispositionと影響検査だけの再実行が分離されているか、既存証拠のcurrent-fit確認が
review status持越しにならないか、`免除`が残っていないか、scope除外とauthorityによる要件変更が
区別されているか、profileを使うreview inputが8項目を完成させるかを確認する。

加えて、安全性・互換性・性能等の定義、任意改善の非blocker化、未決数値、既存authority、
外部namespace、利用者価値の境界がrevision 1から意図せず変わっていないかを確認する。
