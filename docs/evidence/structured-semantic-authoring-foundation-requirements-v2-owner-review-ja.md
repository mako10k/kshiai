# 構造化意味オーサリング基盤 — 要件候補 revision 2 日本語レビュー訳

> この文書は `docs/structured-semantic-authoring-foundation-requirements-v2.md`
> の全レビュー範囲の日本語訳である。英語版が正本であり、本訳はユーザレビュー
> 支援用である。
> Review対象SHA-256: `df4536be8bed752c81164687112041286ebfefc7869fb80e6952f5839e0b556a`

- 状態: Step 1候補、セルフレビュー完了、オーナーレビュー待ち
- 日付: 2026-09-11
- レビュー上の置換対象: structured-semantic-authoring-foundation-requirements-v1.md
- 改訂根拠: revision 1独立レビューと、再試行・退避情報アクセスに関するオーナー訂正
- 意思決定者: Product owner
...
未受理のキャラクタV3作成候補revision 3は参考提案であり、Accepted authorityではない。
そのR14の同一attempt継続、R19の失敗をterminalにしない記述は、採用前に別途レビューする後継候補で
整合させる。ここで過去文書やAccepted ADRを書き換えない。

### 基盤revision 1との差分

revision 2では上記Authority対応表を追加し、途中コンテキストの永続化を必須とせず、
失敗終了と移行元からの再試行を許可する。退避情報の閲覧は移行中のマイグレータに限定する。
その他の基盤要件の対象範囲はrevision 1を維持する。
