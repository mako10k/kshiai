# 旧 cc305 移行経路レビュー（2026-09-25、現行初回試行の根拠には使用しない）

この記録は、当時の移行済み V2 キャラを初回 Stage 試行に使うという誤った計画解釈に基づく履歴である。2026-09-25 に所有者は、初回試行は新規 V3 キャラ Neva と Rio の対戦であり、V2–V3 混在を含めず、V2→V3 移行は後続に残すと明示した。以下の移行 source、focused provider、pointer、rollback の調査は後続 `cc316` の参考に限る。`cc305` の現行範囲と到達条件には適用しない。歴史的な観測・時刻・velocity 実績は保持する。
