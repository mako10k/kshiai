# Character conscious-guidance migration — requirement candidate revision 3

- Status: Step 1 candidate; self-review complete; first owner review pending
- Date: 2026-09-10
- Decision owner: Product owner
- Replaces for review: `character-v2-compatibility-requirements-v2.md`
- Sources: owner `REVISE` route on 2026-09-10; revision-2 independent
...
compare-and-swap current-pointer activation, read-only battle binding, and no
battle-time conversion. Preserve the prohibition on inferred bulk conversion.
A successor ADR may refine only the owner-triggered-upgrade and no-bulk-operation
clauses to authorize this owner-approved, frozen-manifest, deterministic migration.
...
- exclusion of this bounded deterministic bulk migration from accepted scope.

Existing historical V2 generations and battle bindings remain governed by
ADR-0011 and stay readable. Revision 3 does not reinterpret their stored bytes.
...
- `CharacterAgentStateV3` remains mutable battle-runtime agency state.
...
for migration. Requirement revisions 1 and 2 remain historical and unaccepted;
their review state does not carry forward.
...
unchanged. Public prose is not regenerated. Replacement content/provenance
digests and required compiler receipts are recomputed and identify the source
generation and migration contract.
...
generations, compile V3 guidance, and reject mixed contract tuples must be
deployed and verified before selecting schema 3 or executing migration.
...
- Authority tests or review trace the successor decision to ADR-0010 and
  ADR-0011, listing every preserved and refined clause above.
- Namespace tests distinguish `CharacterDefinitionV3` from
  `BattleAssetManifestV3`, `CharacterBattleCompilerInputsV3`, dialogue V3,
  conscious I/O V3, and `CharacterAgentStateV3`; mixed tuples fail closed.
...
- The exact qualified compiler identifier, physical migration-ledger schema,
  and authoring-policy persistence mechanism remain successor-ADR decisions.
  They must satisfy R1-R18 and may not reuse an existing identity with changed
  meaning.
...
- ADR-0010, ADR-0011, ADR-0027, and ADR-0028 authority is explicitly preserved,
  ...
  authority.
- Every existing material V3 namespace identified by the revision-2 review is
  separated from `CharacterDefinitionV3`.
- Direct engine/psyche flow is distinguished from indirect consequences of a
  conscious action choice.
- Frozen descriptive metadata is mapped losslessly; transaction and cutover
  requirements state observable results without fixing a physical database design.
...
Review the exact revision-3 candidate for: complete ADR-0010/0011 disposition;
complete separation from ADR-0028 V3 namespaces; lossless R7 metadata mapping;
