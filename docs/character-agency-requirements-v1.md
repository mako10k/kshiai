# 初回キャラ主体性改修 — 要件候補 revision 1

状態: Proposed / requirement-authority-review step 1（作成・セルフレビュー）。2026-09-09。
CA-01＋最小CA-02。この文書は未承認であり、実装許可ではない。
文書revision 1、候補の状態contract V1、提案するdialogue schemaVersion 3は別の識別子。
正確なSHA-256と独立レビュー入力は同名の `-review-input.md` に記録する。

## 1. authorityと範囲

出典はユーザーの「性格・関係から初期目標」「能力・戦況に基づく行動と対応する発話」
の要望、Accepted ADR-0027 revision 1、ADR-0025/0026、immutable assetの既存規約。
...
R1: 初期目標は同じ顕在意識が作る。作者が明示したdefaultObjectiveがある場合はそれを
拘束条件とする。ない場合は性格・価値・認知済み関係から導出し、根拠がなければ勝利を
通常の目的とする。現行の自動生成victory fallbackを「作者の明示目標」と誤認しない。
作者の禁止規範は目標やモデルの判断で解除できない。関係や性格は未知属性を補わない。
...
初期目標の根拠は作者profile/価値/既知関係/default勝利のID、意図の根拠はこれに加えて
...
作者指定goalの有無を保つ投影を追加し、fallback生成で出典を失わない。
...
AC1: R1の優先関係と出典をテストし、自己性格・既知関係差分を初期入力へ渡す。
...
独立レビューではこの4点とR1の優先関係、schema世代の既存所有、再判断整合を優先確認する。
