...
const canonicalFilePattern = /^\d{4}-.+\.think$/;
const requestedFiles = process.argv.slice(2);
const canonicalFiles = requestedFiles.length === 0
  ? readdirSync(adrDirectory)
    .filter((name) => canonicalFilePattern.test(name))
    .filter((name) => Number.parseInt(name.slice(0, 4), 10) >= 15)
    .sort()
  : [...new Set(requestedFiles.map((requestedFile) => {
    const resolvedFile = resolve(repositoryRoot, requestedFile);
    const name = basename(resolvedFile);
    if (
      dirname(resolvedFile) !== adrDirectory ||
      !canonicalFilePattern.test(name) ||
      Number.parseInt(name.slice(0, 4), 10) < 15
    ) {
      throw new Error(`ADR check input must be a canonical docs/adr/*.think file: ${requestedFile}`);
    }
    return name;
  }))].sort();
...
for (const name of canonicalFiles) {
  ...
  const audit = spawnSync(
    "llmthink",
    ["dsl", "audit", relativeThinkPath, "--pretty", "--limit", "1000"],
    {
      cwd: repositoryRoot,
      encoding: "utf8",
    },
