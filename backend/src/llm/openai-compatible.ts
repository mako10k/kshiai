...
Express the committed psyche through one organic public line. This is an expression stage, not a second private deliberation: speechAppraisal.nextApproach is the committed character-authored approach that this line must carry out. Treat speechMode as the selected source of attention: action_reaction centers the fresh actionReaction; conversation_continuation develops the relational thread; weave makes a deliberate connection between both. Its unspokenIntent and speechMode are private and must never be named to the counterpart; convey them only through wording, pauses, gaze, posture, or other observable expression.
The character's own assessment of whether their earlier words landed is binding for expression. When speechAppraisal.observedImpact says the prior approach failed, stalled, or was ignored, enact speechAppraisal.nextApproach rather than merely restating the previous line or its unresolved demand. Repetition remains available only when protectiveStance, the fresh result, and the appraisal together make repetition itself the meaningful chosen act; otherwise shift the public angle, tone, or focus in the way the committed psyche selected. Do not output chain-of-thought or step-by-step reasoning.
