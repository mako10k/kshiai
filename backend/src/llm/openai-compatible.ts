...
const CHARACTER_DEFINITION_FILL_RESPONSE_FORMAT =
  characterDefinitionFillResponseFormat();
...
When focus permits innerDigests and a non-empty interior conclusion is supplied, weave at least one concise inner beat into the prose without exposing chain-of-thought. If a character spent the turn reflecting and their interior is visible, surface their analysis or guideline as that inner beat. When focus does not grant that interior, keep the public observation opaque (they appear to be thinking or pausing) without inventing private content. External focus must remain observable-only.`;
...
- rest: {"kind":"rest"}
- wait: {"kind":"wait"}
- reflect: {"kind":"reflect","reflectionAnalysis":string,"reflectionGuideline":string}
- free_action: {"kind":"free_action","description":string,"desiredOutcome"?:string,"subjectRefs":string[],"opportunityId"?:string}
description, desiredOutcome, subjectRefs, and opportunityId are free_action-only. They make skill, basic_attack, defend, rest, wait, or reflect invalid even when copied from an available action description. reflectionAnalysis and reflectionGuideline are reflect-only and must both be present when kind is reflect. useFinisher and skillId are skill-only. instrumentRef is allowed only for skill, basic_attack, or defend.
Choose reflect only when the fight looks unfavorable, the character is cautious/deliberative, or other available actions look thin. Never choose reflect for short-tempered or impulsive personalities. Reflect spends the whole turn: no damage, only private analysis and a forward guideline written into battle-volatile memory (not durable character matchup memory).`;
...
dynamics. ${upgrade
    ? "Do not invent history, causes, relationships, or capabilities the source does not support. Prefer null or [] over fabrication."
...
Return JSON only: {"delta": {"interior":{"speechAppraisal":{"anticipatedImpact":"","observedImpact":"","anticipatedSocialConsequence":{"bearer":"self|relationship","meaning":""},"observedSocialConsequence":{"bearer":"self|relationship","meaning":""},"nextApproach":"","continuityPosture":"opening|developing|fraying|deliberate_hold|withdrawing","continuityBasis":{"kind":"fresh_leverage|social_reappraisal|protective_hold|withdrawal","reason":""},"continuityDecision":"advance|reframe|reiterate|withhold"}}, optional persistent private fields and dialogueThread {topic, unresolvedMove, anchoredExchange|null}}, "expressionBrief": {"sourceThread":"action_reaction|conversation_continuation|weave", "continuityDecision":"advance|reframe|reiterate|withhold", "focus":[one or two of self_result,counterpart_result,ambient_change,counterpart_speech], "observedImpact":"", "relationshipMove":"", "publicAim":""}, optional "observableManifestations":[{"modality":"movement|posture|expression|voice","proposal":"","sourceEventIds":[]}], optional "narrativeCues":[{"access":"self_inner|omniscient","description":"","sourceEventIds":[]}]}. A manifestation is only a proposed outward detail for the following expression; it is not yet an observation or event. Copy one or more exact sourceEventIds from turnObservation and never name a hidden cause, trait label, score, or private state in it. A narrative cue is narrator-only, uses exact sourceEventIds, and must not restate raw private state or control labels. Omit both arrays when no grounded cue is useful. Compare the prior anticipation, its observed social consequence, and the present result before selecting the brief. relationshipMove and publicAim are semantic intentions, never a phrase to quote or require. Never invent mechanics, hidden identity, location, or numeric results.`,
    ...
    try {
      ...
      const phaseRule = input.phase === "prologue"
        ...
        : input.phase === "aftermath"
          ? "This is the aftermath. Record one concise matchup-specific reflection and reusable lesson in privateMemory. previous.battleVolatileMemory is ephemeral mid-fight scratch (e.g. reflect notes) you may draw on; do not dump it wholesale into privateMemory. Do not plan another action."
          : "Keep the existing direction unless fresh, observer-safe evidence warrants a change. Mid-fight reflect notes live in battleVolatileMemory only.";
      const data = (await this.chatJson(
